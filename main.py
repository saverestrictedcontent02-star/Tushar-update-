...
async def text_to_txt(client, message: Message):
    ...
    try:
        ...
        custom_file_name = file_name_msg.text.strip() or "output"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'url' in response_data and response_data['url']:
                ...
                return response_data['url'], is_drm, None
    ...
    for i in range(arg - 1, len(links)):
        try:
            ...
            # ClassPlus DRM
                ...
                # Create key file
                key_file = f"{name}_keys.txt"
                with open(key_file, 'w') as f:
                    for key in keys:
                        f.write(f"--key {key}\n")
                
                # Download encrypted
                cmd = f'yt-dlp --allow-unplayable-formats --external-downloader aria2c "{mpd_url}" -o "{name}_encrypted.mp4"'
                os.system(cmd)
                
                await m.reply_text("🔓 **DRM video downloaded. Manual decryption may be required.**")
                count += 1
                continue
