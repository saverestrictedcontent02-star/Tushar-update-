...
async def process_classplus_url(url, name, m, raw_text2, raw_text4):
    ...
    try:
        ...
        if is_drm:
            # Try official ClassPlus API to get signed URL (might bypass DRM)
            headers = {
            ...
            if response.status_code == 200:
                ...
                if 'url' in response_data and response_data['url']:
                    # Success! Return the signed URL (non-DRM)
        ...
        return None, False, "❌ **Failed to process ClassPlus URL**"
...
async def upload(bot: Client, m: Message):
    ...
    try:
        ...
        os.remove(x)
        if not links:
            return await m.reply_text("😶 **Invalid File Input** 😶")
    except:
        os.remove(x)
        return await m.reply_text("😶 **Invalid File Input** 😶")
    ...
    for i in range(arg - 1, len(links)):
        try:
            ...
            # ClassPlus (DRM & Non-DRM)
                ...
                'tencdn.classplusapp',
                'videos.classplusapp',
                'webvideos.classplusapp.'
            ]):
                processed_url, is_drm, error = await process_classplus_url(url, f"{count:03d}) {links[i][0]}", m, raw_text2, raw_text4)
                if error:
            ...
            # ClassPlus DRM
                ...
                cmd = f'yt-dlp --allow-unplayable-formats --fragment-retries infinite --external-downloader aria2c "{mpd_url}" -o "{output_file}"'
                
                try:
                    os.system(cmd)
                    
                    if os.path.exists(output_file) and os.path.getsize(output_file) > 0:
                        await m.reply_text("📤 **Uploading DRM video...**")
                        
                        # Check for mp4decrypt
                        if shutil.which("mp4decrypt") and keys:
                            try:
                                key = keys[0]
                                key_id, key_value = key.split(":")
                                decrypt_cmd = f'mp4decrypt --key {key_id}:{key_value} "{output_file}" "{name}_decrypted.mp4"'
                                result = subprocess.run(decrypt_cmd, shell=True, capture_output=True, text=True)
                                
                                if result.returncode == 0 and os.path.exists(f"{name}_decrypted.mp4"):
                            ...
                            await helper.send_vid(bot, m, cc + "\n\n⚠️ **Encrypted (install mp4decrypt to auto-decrypt)**", output_file, thumb, name, None)
                            os.remove(output_file)
                    else:
                        await m.reply_text("❌ **Failed to download DRM video - file empty**")
                        
                except Exception as e:
                    await m.reply_text(f"❌ **Error processing DRM video**: {str(e)}")
            ...
            # Videos
                ...
                progress_text = (
                    f'<blockquote> 🚀 𝐏𝐑𝐎𝐆𝐑𝐄𝐒𝐒 🚀 {(count/len(links)*100):.2f}%</blockquote>\n\n'
